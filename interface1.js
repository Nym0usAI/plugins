(function () {
    ...
    function shouldUseNewInterface(object) {
        ...
        // Убрана проверка премиума — интерфейс доступен всем
        return true;
    }
...
// ==UserScript==
// @name         Lampa Poster Layout Control
// @version      1.1.0
// @description  Switch between vertical and horizontal posters in Lampa
// @author       ChatGPT
// ==/UserScript==
...
(function () {
    ...
    Lampa.SettingsApi.addParam({
        ...
        field: {
            name: 'Ориентация постеров',
            description: 'Вертикальные или горизонтальные карточки'
        },
        onChange: function () {
    ...
    Lampa.SettingsApi.addParam({
        ...
        field: {
            name: 'Пропорции вертикальных',
            description: 'Высота вертикальных постеров'
        },
        onChange: function () {
            Lampa.Settings.update();
            applyPosterStyle();
    ...
    function applyPosterStyle() {
        ...
        let orientation = Lampa.Storage.field('PosterOrientation') || 'vertical';
        let ratio = Lampa.Storage.field('PosterRatio') || '150';
        ...
        let style = `
            <style id="custom-poster-style">
                ${posterCSS}
            </style>
        `;

        $('#custom-poster-style').remove();
        $('head').append(style);
    }
