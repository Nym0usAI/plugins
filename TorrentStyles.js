(function () {
    ...
    // Обертка для регистрации плагина в Lampa
    function startPlugin() {
        ...
        // Используем Lampa Listener для отслеживания изменений, если возможно, но MutationObserver надежнее для модов
        var observer = new MutationObserver(updateTorrentStyles);
    ...
    if (window.Lampa) {
        Lampa.Plugins.add({
            ...
            'type': 'visual',
            'version': '2.0.4-custom',
            'description': 'Цветовая индикация сидов и битрейта',
            'author': 'Custom',
            'on_start': startPlugin
        });
    } else {
        // Fallback если запущен не в Lampa или старая версия
        startPlugin();
    }
