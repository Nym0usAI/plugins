...
(function() {
    ...
    function applyChanges() {
        ...
        // Сначала скрываем все существующие кнопки
        targetContainer.find('.full-start__button').not('.button--edit-order').addClass('buttons-plugin-hidden-temp');
                ...
                if (item.type === 'folder') {
                    ...
                    if (buttonsInFolders.indexOf(btnId) === -1) {
                        ...
                        if (btn && !btn.hasClass('hidden')) {
                            btn.removeClass('buttons-plugin-hidden-temp');
        ...
        // Удаляем временно скрытые кнопки
        targetContainer.find('.buttons-plugin-hidden-temp').remove();
    ...
    function reorderButtons(container) {
        ...
        // Временная маска для скрытия оригинальных кнопок
        targetContainer.addClass('buttons-plugin-processing');
        ...
        setTimeout(function() {
            targetContainer.find('.buttons-plugin-hidden-temp').remove();
            targetContainer.removeClass('buttons-plugin-processing');
        }, 50);
    ...
    function init() {
        var style = $('<style>' +
            ...
            'position: relative;' +
            '}' +
            '.full-start-new__buttons.buttons-plugin-processing { ' +
            'min-height: 60px;' +
            '}' +
            '.buttons-plugin-hidden-temp { ' +
