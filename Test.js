(function () {
    ...
    const DEFAULT_LABELS = {
      ...
      'button--shots': () => 'Скриншоты',
      'text:скриншоты': () => 'Скриншоты',
      'text:shots': () => 'Скриншоты',
      'text:кадры': () => 'Скриншоты'
    };
    ...
    // ИСПРАВЛЕННАЯ ФУНКЦИЯ: Извлекает уникальный ключ кнопки
    function extractButtonKey($element) {
      // 1. Проверяем специальные data-атрибуты
      const dataId = $element.data('id') || $element.attr('data-id');
      const dataName = $element.data('name') || $element.attr('data-name');
      const dataAction = $element.data('action') || $element.attr('data-action');
      const dataType = $element.data('type') || $element.attr('data-type');
      
      if (dataId) return `data-id:${dataId}`;
      if (dataName) return `data-name:${dataName}`;
      if (dataAction) return `data-action:${dataAction}`;
      if (dataType) return `data-type:${dataType}`;
      
      // 2. Проверяем стандартные классы Lampa
      const classes = ($element.attr('class') || '').split(/\s+/);
      
      // Ищем стандартные классы кнопок
      ...
      // 3. Проверяем другие возможные классы
      ...
      // 4. Проверяем текст кнопки - ОСОБОЕ ВНИМАНИЕ ДЛЯ SHOTS
      const text = $element.text().trim();
      const lowerText = text.toLowerCase();
      
      // Специальная проверка для кнопки shots
      if (lowerText.includes('shot') || lowerText.includes('скриншот') || 
          lowerText.includes('кадр') || lowerText.includes('screenshot')) {
        return 'button--shots';
      }
      
      if (text && text.length < 30) { // Ограничиваем длину текста
        const cleanText = text.toLowerCase().replace(/\s+/g, '_').replace(/[^\wа-я_]/gi, '');
        if (cleanText) {
          return `text:${cleanText}`;
        }
      }
      
      // 5. Проверяем иконку (по классам svg)
      const svgClasses = $element.find('svg').attr('class') || '';
      if (svgClasses) {
        const svgClass = svgClasses.split(/\s+/).find(c => c.includes('icon--'));
        if (svgClass) return svgClass;
      }
      
      // 6. Комбинированный ключ из нескольких признаков
      const combinedKey = [
        classes.filter(c => !['selector', 'focus', 'hide'].includes(c)).join('-'),
        text.substring(0, 20)
      ].filter(Boolean).join('|');
      
      return `combo:${Lampa.Utils.hash(combinedKey)}`;
    }
    ...
    function extractButtonLabel(key, $element) {
      ...
      if (key.startsWith('text:')) {
        const textFromKey = key.substring(5).replace(/_/g, ' ');
        return textFromKey.charAt(0).toUpperCase() + textFromKey.slice(1);
      }
