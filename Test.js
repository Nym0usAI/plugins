 ...
 * Автор: @Cheeze_l
 * 
 * Исправление: мгновенное применение изменений без задержек
 */
...
(function() {
    ...
    function applyButtonDisplayModes(buttons) {
        buttons.forEach(function(btn) {
            ...
            if (hasTextContent) {
                ...
                // Получаем все текстовые ноды и span элементы
                var nodesToWrap = [];
                btn.contents().each(function() {
                ...
                // Немедленно применяем стили для текущего режима
    ...
    function applyChanges() {
        ...
        // Убрана задержка, обновление навигации сразу
        if (currentContainer) {
    ...
    function refreshController() {
        ...
        // Убрана задержка, обновление сразу
        try {
            Lampa.Controller.toggle('full_start');
            if (currentContainer) {
                setupButtonNavigation(currentContainer);
            ...
            // Пробуем еще раз через микро-задержку если первая попытка не удалась
            setTimeout(function() {
                try {
                    Lampa.Controller.toggle('full_start');
                } catch(e2) {}
    ...
    function init() {
        var style = $('<style>' +
            ...
            '' +
            '/* Плавные переходы для мгновенного отображения */' +
            '.full-start__button .text-wrapper, .full-start__button > span:not(.text-wrapper) { transition: opacity 0.2s ease, display 0.2s ease; }' +
            '</style>');
        ...
        Lampa.Listener.follow('full', function(e) {
            ...
            // Уменьшена задержка инициализации
            setTimeout(function() {
                try {
            ...
            }, 300); // Уменьшено с 400ms до 300ms
        });
    }
